...
The recorded source-report audit reported zero overlap across train, validation, and test partitions for patient, lesion, exact-duplicate, and split-group identities. Perceptual-hash review covered all flagged near-duplicate pairs. These checks reduce known identity leakage; they do not prove that no unrecorded biological relation exists between all images.

\subsection{Repair of contaminated legacy evidence}

Clean-v2 used the same nominal total of 2,869 rows but contained three patient identifiers and 13 rows crossing partitions. All Loop170 results tied to this split are classified as \path{legacy_patient_contaminated}. They remain useful for operational traceability and for illustrating how conclusions change after an audit, but they are excluded from \cleanv{} ranking and from the main conclusion.

\input{tables/evidence_scope}

\subsection{Partition access}

Loop191 and Loop192 used the \cleanv{} validation partition during iterative development, model or checkpoint selection, and promotion decisions. Their registry class \texttt{protected\_validation} records access scope; it does not make the partition untouched confirmatory evidence. We therefore describe these results as adaptive development and checkpoint-selection validation, with corresponding selection optimism. Neither loop opened the 430-row test-v3 partition. Loop206 used only a train-derived pilot: 384 independent training groups were divided into 308 fit groups and 76 train-screen holdout groups. All protected \cleanv{} validation, test-v3, and PH2 paths remained sealed throughout Loop206.
